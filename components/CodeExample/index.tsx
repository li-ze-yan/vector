...
import theme from "../../lib/syntax-highlighter/theme.json";
import { CopyButton } from "../CopyButton";

import { highlightClasses } from "../../lib/highlight-classes";
import { stripShikiComments } from "../../lib/shiki";
import atApplyInjection from "../../lib/syntax-highlighter/at-apply.json";
import atRulesInjection from "../../lib/syntax-highlighter/at-rules.json";
import themeFnInjection from "../../lib/syntax-highlighter/theme-fn.json";
  ...
  const code = highlighter
    .codeToHtml(codeWithoutPrettierIgnore, {
      ...
      transformers: [
        transformerNotationHighlight({
          classActiveLine:
            "-mx-5 pl-[calc(var(--spacing)*5-2px)] border-l-2 pr-5 border-sky-400 bg-sky-300/15",
        }),
        transformerNotationDiff({
          classLineAdd:
...
function CodeExampleFilename({ filename }: { filename: string }) {
  return (
    <div className="px-3 pt-0.5 pb-1.5 text-xs/5 text-gray-400 dark:text-white/50">{filename}</div>
  );
}
