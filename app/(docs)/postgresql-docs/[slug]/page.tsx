...
export async function generateStaticParams() {
  const slugs = await getDocPageSlugs("postgresql-docs");
  return slugs?.map((slug) => ({ slug }));
}

export async function generateMetadata(props: DocsProps): Promise<Metadata> {
  const params = await props.params;
  const post = await getDocPageBySlug(params.slug, "postgresql-docs");
...
export default async function DocPage(props: DocsProps) {
  ...
  const [post, tableOfContents] = await Promise.all([
    getDocPageBySlug(params.slug, "postgresql-docs"),
    generateTableOfContents(params.slug, "postgresql-docs"),
  ]);
